...
export class TileDragController extends EventTarget {
    ...
    addSource(source : TileDragSource) {
        ...
        if (DEBUG.LOG_MOUSE_POSITION) {
            this.dropTarget.element.addEventListener('mouseover', (evt : PointerEvent) => {
                const cursorPos : Coord = [evt.clientX, evt.clientY];
                console.log('Mouse cursor:',  cursorPos,
                            'Grid coordinates:', this.dropTarget.screenPositionToGridPosition(cursorPos),
                            'Triangle on grid:', this.dropTarget.screenPositionToTriangleCoord(cursorPos));
            });
        }
    }
    ...
    onDragEnd(context : TileDragSourceContext, evt : DragEvent) : boolean {
        ...
        if (fixedTriangle) {
            successful = this.dropTarget.dropTile(context.source, { fixed: fixedTriangle, moving: movingTriangle });
        }
