...
export function shiftCoordinates2(coords: Coord[][], shift: Coord): Coord[][] {
    return coords.map((g) =>
        g.map((coord) => [coord[0] + shift[0], coord[1] + shift[1]]),
    );
}
 ...
 */
export function shiftToAndReturnOrigin(
...
export function shiftToOrigin2(coords: readonly Coord[][]): Coord[][] {
    const minX = Math.min(
        ...coords.map((g) => Math.min(...g.map((c) => c[0]))),
    );
    const minY = Math.min(
        ...coords.map((g) => Math.min(...g.map((c) => c[1]))),
    );
...
export function pointInTriangle(
    pt: Coord,
    [v1, v2, v3]: readonly [Coord, Coord, Coord],
): boolean {
