...
export function makeConvexHull(points: Coord[]) : Coord[] {
	let newPoints = points.map((p) => ({ x: p[0], y: p[1] }));
	newPoints = convexhull.makeHull(newPoints);
	return newPoints.map((p) => [p.x, p.y]);
}
