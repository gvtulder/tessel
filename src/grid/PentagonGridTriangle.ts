...
const height =
    Math.cos(((60 - 45) / 180) * Math.PI) * Math.sqrt(2 * RADIUS * RADIUS);
const base =
    2 * Math.sin(((60 - 45) / 180) * Math.PI) * Math.sqrt(2 * RADIUS * RADIUS);

const colstepx =
    Math.cos((originalAngle / 180) * Math.PI) * (base + 2 * height);
const colstepy =
    Math.sin((originalAngle / 180) * Math.PI) * (base + 2 * height);
const rowstepx =
    Math.cos(((90 + originalAngle) / 180) * Math.PI) * (base + 2 * height);
const rowstepy =
    Math.sin(((90 + originalAngle) / 180) * Math.PI) * (base + 2 * height);
...
export class PentagonGridTriangle extends Triangle {
    protected calc(x: number, y: number): TriangleParams {
        ...
        const xc =
            xb + Math.cos(((angle - alpha + 90) / 180) * Math.PI) * RADIUS;
        const yc =
            yb + Math.sin(((angle - alpha + 90) / 180) * Math.PI) * RADIUS;
        const xd =
            xc +
            Math.cos(((angle - alpha + 90 + 180 - beta) / 180) * Math.PI) *
                RADIUS;
        const yd =
            yc +
            Math.sin(((angle - alpha + 90 + 180 - beta) / 180) * Math.PI) *
                RADIUS;
        const xe =
            xd +
            Math.cos(((angle - alpha + 90 + 180 - beta + 90) / 180) * Math.PI) *
                RADIUS;
        const ye =
            yd +
            Math.sin(((angle - alpha + 90 + 180 - beta + 90) / 180) * Math.PI) *
                RADIUS;
        ...
        if (rot == 1) {
            ...
            offsetx =
                Math.cos((originalAngle / 180) * Math.PI) * (base + height) -
                (xa + xe) / 2;
            offsety =
                Math.sin((originalAngle / 180) * Math.PI) * (base + height) -
                (ya + ye) / 2;
        }

        offsetx +=
            colstepx * (col + 0.5 * wrapModulo(row, 2) + Math.floor(row / 2)) +
            rowstepx * (row / 2);
        offsety +=
            colstepy * (col + 0.5 * wrapModulo(row, 2) + Math.floor(row / 2)) +
            rowstepy * (row / 2);
        ...
        p.rotationOffsets = [0, 1, 2, 3].map((i) => {
    ...
    protected approxGridPositionToTriangleCoord(gridPos: Coord): Coord {
        // TODO
        return [Math.floor(gridPos[0] * 12), Math.floor(gridPos[1])];
    }
}
