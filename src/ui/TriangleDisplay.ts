...
export class TriangleDisplay {
    ...
    build() {
        ...
        const centerEl = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "circle",
        );
        ...
        const polyString = this.triangle.polyPoints.map(
            (p) => `${(p[0] * SCALE).toFixed(5)},${(p[1] * SCALE).toFixed(5)}`,
        );
        ...
        if (DEBUG.NUMBER_TRIANGLES) {
            ...
            text.appendChild(
                document.createTextNode(
                    `(${this.triangle.x},${this.triangle.y})`,
                ),
            );
            group.append(text);
        }
