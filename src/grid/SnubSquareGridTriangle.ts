...
export class SnubSquareGridTriangle extends Triangle {
    protected calc(x: number, y: number): TriangleParams {
        ...
        const rotSequence = rotSequences.find(
            (seq) => seq && seq.find((edge) => edge && edge[1] == p.shape),
        );
        // find the first edge in the sequence
        const rotIdx = rotSequence.findIndex(
            (edge) => edge && edge[1] == p.shape,
        );
        // start with the right shape
        const orderedSequence = [
            ...rotSequence.slice(rotIdx),
            ...rotSequence.slice(0, rotIdx),
        ];
        p.rotationOffsets = orderedSequence.map((edge) =>
            edge == null
                ? null
                : {
